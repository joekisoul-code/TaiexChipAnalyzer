...
def main(src: str) -> None:
    ...
    ev["range"] = {"ks": [int(k) for k in KS], "index": idx, "assets": assets,
                   "evidence": "iv 當 sigma 的 k 日路徑高低分位：pinball 比 App ATR+EWMA 低 6.7% [−9.9%,−3.9%]、比 rv20 低 12%；各標的校準後比自身 ATR+EWMA 低 4~7.5% (2330 k=20 不顯著)。乘數凍結在 2026-09-24 的滾動 750 日值 (每季人工更新，尚未每日重算與監控觸及率)，含 2023~26 多頭漂移 → 下檔偏淺；指數另附 2017~ 擴張版 (保守)。以加權指數高低點衡量觸及率比名目少 1~2pp (乘數以台指期校準)。k 日窗口內若遇除息 (0050 1/7 月、2330 與 00981A 3/6/9/12 月中)，價位沒有扣股利。",
