...
分層定義 (train() 走動式重算並存 data/models/confidence.json；forecast 時 label() 只做標籤)：
  ...
  base : 高 = 叫牌 且 規律同向；中 = 叫牌 且 規律不反向 且 季線同向；低 = 其餘叫牌
"""
...
def _tier_masks(o: pd.DataFrame, variant: str) -> dict[str, tuple[pd.Series, pd.Series]]:
    ...
    return {"高": ((c >= 1) & (rs > 0), (c <= -1) & (rs < 0)),
            "中": ((c >= 1) & (rs >= 0) & (bull == 1), (c <= -1) & (rs <= 0) & (bull == 0)),
            "低": (c >= 1, c <= -1)}
...
def label(call: str, strength: str, variant: str, night: float | None, rule_score: float, bull: bool | None) -> str | None:
    ...
    if rs_ok_strict:
        return "高"
    if rs_ok and bull is not None and (bull if up else not bull):
        return "中"
    return "低"
...
def train(matrix: pd.DataFrame, pat: dict, write: bool = True, verbose: bool = True) -> dict:
    ...
    for variant in ("base", "night"):
        for h in HORIZONS:
            ...
            o = o.merge(d[["date", "rule_score", "bull", "night"]], on="date", how="left").sort_values("date").reset_index(drop=True)
            o["call"] = 0
